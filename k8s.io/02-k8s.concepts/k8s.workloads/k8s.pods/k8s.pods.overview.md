...
- A Pod represents a unit of deployment: 
        - a single instance of an application in Kubernetes, which might consist of either a single containerA lightweight and 
        - portable executable image that contains software and all of its dependencies. 
...
- Working with Pods
           ...
           - When a Pod gets created (directly by you, or indirectly by a _controller_), it is scheduled to run on a Node in your cluster. 
...
- Pods do not, by themselves, self-heal. If a Pod is scheduled to a Node that fails, or if the scheduling operation itself fails, 
  the Pod is deleted; likewise, a Pod won't survive an eviction due to a lack of resources or Node maintenance. 
...
- Pods and controllers
        ...
        - A controller for the resource handles replication and rollout and automatic healing in case of Pod failure. 
         ...
         - Here are some examples of workload resources that manage one or more Pods:
                    ...
                    StatefulSet - Manages the deployment and scaling of a set of Pods with durable storage and persistent ids for POD and 
                              provides guarantees about the ordering and uniqueness of these Po
...
- Like individual application containers, Pods are considered to be relatively ephemeral (rather than durable) entities. 
   In pod lifecycle, Pods are created, assigned a unique ID (UID), and scheduled to nodes where they remain until termination (according to restart policy) or deletion. 
